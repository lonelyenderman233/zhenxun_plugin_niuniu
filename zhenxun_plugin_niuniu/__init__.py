...
usage：
    ...
    【牛子深度排行】 --查看本群负数牛子深度排行
    【打胶】 --对自己的牛子进行操作，结果随机
    牛子长度<-100进化成魅魔，与正牛子击剑有20%概率将对方牛子吞噬（对牛头人只有10%概率触发），自身长度不变
    牛子长度>100进化成牛头人，击剑有10%概率将对方牛子归0，自身长度不变，但对牛头人及魅魔无效
...
async def _(event: GroupMessageEvent):
  ...
  try:
    ...
    if time_pass < 3600*23:
      ...
      jj_refuse = [
        f'才过去了{int(time_pass/3600)}h,你就又要击剑了，真是饥渴难耐啊，{int(time_rest/3600)+1}h后再来吧',
...
async def _(event: GroupMessageEvent):
  ...
  try:
    ...
    if my_long <= -100:
      ...
      picc = """
      牛头人：
          与所有牛子击剑时有10%的几率触发蛮牛冲撞，将对方牛子归0（对魅魔与牛头人无效）
      """
  except KeyError:
    result = "你还没有牛子呢！"
  finally:
...
async def _(event: GroupMessageEvent):
  ...
  try:
    ...
    if time_pass < 3600*23:
      time_rest = 3600*23 - time_pass
      glue_refuse = [
        f'才过去了{int(time_pass/3600)}h,你就又要打胶了，牛牛受得住吗？{int(time_rest/3600)+1}h后再来吧',
		f'不行不行，你的身体会受不了的，歇{int(time_rest/3600)+1}h再来吧',
